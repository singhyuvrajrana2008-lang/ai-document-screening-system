...
def _run_in_background(screening_id):
    try: run_screening_pipeline(screening_id)
    except Exception: pass

@require_auth
def run_screening(screening_id):
    try:
        response = supabase.table("screenings").select("id, screening_number, status, created_by").eq("id", screening_id).maybe_single().execute()
