...
try:
    ...
    from results import get_ocr_result, get_risk_result, get_screening_result, get_tampering_result, get_validation_result
    from run_screening import run_screening
    from screenings import create_screening, upload_document
except ImportError:
    from backend.config import supabase
    from backend.actions import officer_action
    from backend.history import dashboard_stats, list_screenings
    from backend.results import get_ocr_result, get_risk_result, get_screening_result, get_tampering_result, get_validation_result
    from backend.run_screening import run_screening
    from backend.screenings import create_screening, upload_document
...
def health_check():
    ...
    return jsonify({"success": True, "data": {"status": "operational", "database": db_status}})
...
app.add_url_rule("/api/screenings/<screening_id>/documents", view_func=upload_document, methods=["POST"])
app.add_url_rule("/api/screenings/<screening_id>/run", view_func=run_screening, methods=["POST"])
app.add_url_rule("/api/screenings/<screening_id>", view_func=get_screening_result, methods=["GET"])
