...
def officer_action(screening_id):
    ...
    if action not in ALLOWED_ACTIONS:
        return _error("VALIDATION_ERROR", "action must be approved, manual_review, rejected, or escalated.", 400)
    user_id = g.current_user.id
    try:
        screening_response = (supabase.table("screenings").select("id, screening_number, created_by, status").eq("id", screening_id).maybe_single().execute())
        ...
        status = "completed" if action == "approved" else "manual_review" if action in {"manual_review", "escalated"} else "rejected"
        update_response = (supabase.table("screenings").update({"final_action": action if action != "escalated" else "manual_review", "status": status, "reviewed_by": user_id, "reviewed_at": "now()"}).eq("id", screening_id).execute())
        if not getattr(update_response, "data", None):
            return _error("UPDATE_FAILED", "Unable to update screening action.", 500)
        audit_response = (supabase.table("audit_logs").insert({"screening_id": screening_id, "actor_id": user_id, "action": action, "previous_status": previous_status, "new_status": status, "metadata": {"source": "officer_action"}}).execute())
