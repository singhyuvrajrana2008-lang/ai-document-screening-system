...
def _result_or_404(table, screening_id, label):
    try:
        ...
        return None, _error("INTERNAL_ERROR", "%s result lookup failed." % label, 500)
    if not result:
        return None, _error("RESULT_NOT_FOUND", "%s result is not available." % label, 404)
    return result, None
...
def get_screening_result(screening_id):
    ...
    try:
        ocr = _load_single("ocr_results", screening_id)
        validation = _load_single("validation_results", screening_id)
        tampering = _load_single("tampering_results", screening_id)
        face = _load_single("face_verifications", screening_id)
        risk = _load_single("risk_assessments", screening_id)
    except Exception:
...
def get_ocr_result(screening_id):
    return _stage_result(screening_id, "ocr_results", "OCR", lambda r: {
        "status": r["processing_status"], "confidence": r.get("confidence"), "fields": r.get("extracted_data") or {}
    })


@require_auth
def get_validation_result(screening_id):
    return _stage_result(screening_id, "validation_results", "Validation", lambda r: {
        "status": r["overall_status"], "issues": r.get("issues") or [], "checks": r.get("checks") or {}
    })


@require_auth
def get_tampering_result(screening_id):
    return _stage_result(screening_id, "tampering_results", "Tampering", lambda r: {
        "status": "completed", "tampering_detected": r["tampering_detected"], "score": r.get("tampering_score"),
        "indicators": r.get("indicators") or [], "explanation": r.get("explanation") or ""
    })


@require_auth
def get_risk_result(screening_id):
    return _stage_result(screening_id, "risk_assessments", "Risk", lambda r: {
        "score": r["risk_score"], "level": r["risk_level"], "factors": r.get("contributing_factors") or []
    })
