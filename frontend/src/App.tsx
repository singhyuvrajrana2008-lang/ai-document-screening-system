...
function mapStatus(status: string, action?: string): ScreeningRecord['status'] {
  ...
  if (value === 'manual_review' || value === 'escalated') return 'Manual Review';
...
function mapApiRecord(item: Awaited<ReturnType<typeof getScreenings>>['items'][number]): ScreeningRecord {
  ...
  return {
    ...
    timeDisplay: Number.isNaN(created.getTime()) ? '—' : created.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }),
    ocrConfidence: 0,
...
export default function App() {
  ...
  useEffect(() => {
    ...
    Promise.allSettled([
      ...
      setBackendStatus(healthResult.status === 'fulfilled' && healthResult.value ? 'connected' : 'offline');
  ...
  if (currentScreen === 'auth') {
    return <AuthScreen onLoginSuccess={() => setCurrentScreen('command-center')} />;
  }
