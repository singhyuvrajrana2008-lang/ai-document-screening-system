...
export default function App() {
  ...
  const refreshData = async () => {
    try {
      ...
      setBackendStatus(health.status === 'operational' ? 'connected' : 'offline');
      setStats(dashboard);
      setRecords(history.items.map((item) => ({
        id: item.screening_id,
        officerId: 'CURRENT', officerName: 'Current Officer',
        documentType: item.document_type || 'Unknown', country: '-',
        ...
        status: item.action === 'approved' ? 'Approved' : item.action === 'rejected' ? 'Rejected' : 'Pending',
        timestamp: item.created_at || new Date().toISOString(),
        timeDisplay: item.created_at ? new Date(item.created_at).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) : '-',
        ocrConfidence: 0, formatValidation: 'PASS', formatStandard: 'ICAO DOC 9303',
        tamperingIndex: 0, faceMatchScore: 0, contributingFactors: [],
        validationChecks: { mrzValid: false, hologramValid: false, faceMatchValid: false, watchlistClean: false },
        ocrData: { fullName: '', dob: '', expiryDate: '', documentNumber: '', nationality: '', documentType: item.document_type || '' },
        auditTrail: []
      })));
    } catch {
  ...
  const handleSignOut = () => setCurrentScreen('auth');
  ...
  const handleSelectRecord = (record: ScreeningRecord | null) => {
    ...
    if (record) setCurrentScreen('audit-history');
  };

  if (currentScreen === 'auth') return <AuthScreen onLoginSuccess={handleLoginSuccess} />;

  return (
    <div className="min-h-screen bg-[#09090B] text-[#A1A1AA] font-sans flex antialiased">
      ...
      <div className="flex-1 ml-64 flex flex-col min-h-screen relative z-10">
        ...
        <footer className="h-9 bg-[#09090B] border-t border-[#27272A] flex items-center justify-between px-4 text-[11px] font-mono text-[#71717A] shrink-0">
          <span>BACKEND: <span className={backendStatus === 'connected' ? 'text-[#10B981]' : 'text-[#F43F5E]'}>{backendStatus.toUpperCase()}</span></span>
          <span>API: http://localhost:5000/api</span>
        </footer>
      </div>
    </div>
  );
