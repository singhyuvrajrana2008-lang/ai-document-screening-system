...
export interface ScreeningResult {
  screening_id: string;
  document?: { document_id?: string; document_type?: string; filename?: string };
  ocr?: { status?: string; confidence?: number; fields?: Record<string, string> };
  validation?: { status?: string; issues?: string[]; checks?: Record<string, string> };
  tampering?: { status?: string; tampering_detected?: boolean; score?: number; indicators?: string[]; explanation?: string };
  face_verification?: { status?: string; similarity_score?: number; message?: string };
  risk?: { score?: number; level?: string; factors?: string[]; explanation?: string };
...
async function request<T>(path: string, options: RequestInit = {}): Promise<T> {
  ...
  const body: ApiResponse<T> = await response.json().catch(() => ({ success: false, error: { code: 'INVALID_RESPONSE', message: 'Backend returned invalid JSON.' } }));
...
export const api = {
  ...
  createScreening: (file: File, documentType: string) => {
    const form = new FormData(); form.append('document', file); form.append('document_type', documentType);
    return request<{ screening_id: string; status: string; document_id: string }>('/screenings', { method: 'POST', body: form });
  },
  uploadDocument: (screeningId: string, file: File, documentType: string) => {
    const form = new FormData(); form.append('document', file); form.append('document_type', documentType);
    return request<{ document_id: string; screening_id: string; status: string }>(`/screenings/${screeningId}/documents`, { method: 'POST', body: form });
  },
