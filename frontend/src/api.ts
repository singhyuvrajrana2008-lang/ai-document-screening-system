...
export const api = {
  ...
  createScreening: (file: File, documentType: string) => { const form = new FormData(); form.append('document', file); form.append('document_type', documentType); return request<{ screening_id: string; status: string; document_id: string }>('/screenings', { method: 'POST', body: form }); },
  uploadDocument: (screeningId: string, file: File, documentType: string) => { const form = new FormData(); form.append('document', file); form.append('document_type', documentType); return request<{ document_id: string; screening_id: string; status: string }>(`/screenings/${screeningId}/documents`, { method: 'POST', body: form }); },
